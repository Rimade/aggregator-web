		...
		<div className="min-h-dvh bg-white">
				...
				<div className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-4 sm:px-6 sm:py-5">
					<div className="flex items-center gap-3">
						...
						<div className="leading-tight">
							<div className="text-sm font-semibold tracking-wide text-slate-900">Aggregator</div>
			...
			<main className="mx-auto w-full max-w-6xl px-4 pb-14 pt-8 sm:px-6 sm:pb-16 sm:pt-10">
				<section className="grid items-center gap-10 lg:grid-cols-2">
					<div>
						...
						<p className="mt-4 max-w-xl text-pretty text-base leading-7 text-slate-600">
							Вы вручную добавляете кафе, они платят за доступ. Гости оформляют заказ по QR, а
							сотрудники видят заказы в панели. В MVP деньги не проходят через платформу.
						</p>
						...
						<div className="rounded-3xl bg-white p-4 ring-1 ring-slate-200 shadow-[0_30px_80px_-40px_rgba(15,23,42,0.35)]">
							...
							<div className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
								<div className="flex items-start justify-between gap-3">
									<div>
										<div className="text-sm font-medium text-slate-900">Кафе “Demo”</div>
				...
				<section className="mt-14 grid gap-4 md:grid-cols-3">
					{features.map((f) => (
						<div key={f.title} className="rounded-3xl bg-white p-5 ring-1 ring-slate-200 shadow-sm">
							<div className="text-sm font-semibold text-slate-900">{f.title}</div>
							<div className="mt-2 text-sm leading-6 text-slate-600">{f.description}</div>
						</div>
					))}
				</section>
...
function LineItem({ name, qty, price }: { name: string; qty: number; price: string }) {
	return (
